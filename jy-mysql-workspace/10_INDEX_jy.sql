/*
    ## INDEXT ##
    1. 색인객체
    2. SQL 명령문의 처리 속도를 향상시키기 위한 객체로 데이터를 빠르게 조회할 수 있는 포인터 제공
    ...
    5. 장점
        - 데이터 검색시 전체 테이블을 검색하는 대신 인덱스를 통해 검색하믈 속도가 향상됨
        - 시스템에 걸리는 부하를 줄여 시스템 전체 성능을 향상시킬 수 있음
    6. 단점
        - 인덱스 생성을 위한 추가 저장 공간이 필요함
        - 인덱스 생성시 기간ㅇ 걸림
        - 데이터가 변경(INSERT, UPDATE, DELETE)될 때 마다 인덱스도 갱신해야됨
        => 즉, 변경작업이 빈번한 테이블에 INDEX생성시 오히려 성능 저하가 발생될 수 있음

    ##INDEX 적용예##
    1. 어떤 컬럼에 인덱스를 만들면 좋을까?
        - 중복된 데이터값들이 없는 고유한 데이터값을 가지는 컬럼에 만드는게 제일 젛으
            => 선택도(selectvity)가 좋은 컬럼이라함
